...
<head>
	...
	<script type="text/javascript">
		var static = {
			pronuncationType: ["", "單讀音字", "破音字", "異讀字", "異讀破音字"]
		};
		...
		function init() {
			if (window.navigator.standalone == true) {
				...
				document.getElementById("title").style.lineHeight = "28px";
				document.getElementById("title").style.verticalAlign = "bottom";
			}
		}
		...
		function requestCallbackCharacter(xhttp) {
			...
			if (xhttp.readyState == 4 && xhttp.status == 200) {
				...
				document.getElementById("infoPronunciationType").innerText = static.pronuncationType[parseInt(result.baseInfo.pronunciation_type)];
		...
		function searchByCharacter(char) {
			...
			ajax("https://crossorigin.me/http://humanum.arts.cuhk.edu.hk/Lexis/lexi-can/search.php?q=" + encodeBig5URIComponent(char), requestCallbackCharacter);
		}
		...
		function search(e) {
			...
			if (!JSON.parse(divSearchType.getAttribute("isSearchByCharacter"))) {
				...
				if (selectConsonant.options[selectConsonant.selectedIndex].value != "-" || selectVowel.options[selectVowel.selectedIndex].value != "-") {
					ajax("https://crossorigin.me/http://humanum.arts.cuhk.edu.hk/Lexis/lexi-can/pho-rel.php?s1=" + selectConsonant.options[selectConsonant.selectedIndex].value + "&s2=" + selectVowel.options[selectVowel.selectedIndex].value + "&s3=" + selectTone.options[selectTone.selectedIndex].value, requestCallbackPronuncation);
				}
